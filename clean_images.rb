...
DIRECTORY = ENV.fetch('HABITAT',
                      '~/Users/kimberlygarmoe/repos/habitat/components/docs-chef-io')

# initialize set
# used = Set.new
# images = Set.new
search_text = %r{images}
# fileutils.
# create an array of images used in the docs
# clean up the elements
Dir.glob("#{DIRECTORY}  /content/habitat/*.md").each do |file|
  File.readlines(file).each do |line|
    if line =~ search_text then
      # A lot of work to clean up the file names.
      # line = line.sub("/images/", '')
